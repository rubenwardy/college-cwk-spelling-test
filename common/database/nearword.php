...
class Nearword {
	...
	public function Nearword($db) {
		if ($db){
			// Get values from database record
			$this->id = $db->nearwordID;
	...
	public function save(){
		...
		if ($this->id == -1){
			// Create SQL query
			$h = $handle->prepare("INSERT INTO nearword(wordID,word) VALUES (?,?)")  or die("SQL Prepare: ".mysqli_error($handle));
			$h->bind_param('is',$this->wordID,$this->word) or die("SQL Param: ".mysqli_error($handle));
			...
			function update($id, $col, $value, $int=false){
				...
				if ($int){
					...
					$res = "UPDATE nearword SET $col = $value WHERE nearwordID = $id";
				}else{
					// Update string field
					$res = "UPDATE nearword SET $col = '$value' WHERE nearwordID = $id";
				}
	...
	// Get a test by their nearwordID
	public static function get($id){
		...
		$res = $handle->query("SELECT * FROM nearword WHERE wordID = $id") or die("Query Error: ".mysqli_error($handle));
	...
	public static function _search($query){
		...
		$res = $handle->query("SELECT * FROM nearword ".$query);
		if (!$res){
			return;
		}
		...
		while($obj = $res->fetch_object()){ 
			array_push($result,new Nearword($obj));
		}
		return $result;
	}
